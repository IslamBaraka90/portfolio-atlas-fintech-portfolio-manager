...
Clock dependence: the original Chapter 6 valuation journey freezes a 2026-09-15 fixture price with a 10-day age limit and began failing at 2026-09-25 13:30Z on the real clock; a 30-day stopgap only postponed the failure to mid-October. Task 6 pins time for browser journeys instead. `ATLAS_CLOCK_START` (test-only, refused together with `DEMO_CACHE_PATH`) starts the API clock at a fixed instant and lets it run in real time; `playwright.config.ts` sets it to 2026-09-22T15:00:00Z, a Tuesday during New York hours after the fixtures. The `tests/helpers/clock.ts` fixture reads `metadata.generatedAt` from the server before each journey, installs that time in the page and exposes it as `serverNow()`. All journey timestamps, seed helpers and the in-process governance server use it in place of the runner's wall clock, and derived times can only lag the server by one round trip. The valuation journey is back to its lesson values (default 10-day age, 1-day stale check, 10-day restore): the 2026-09-15 row is 7 days old at the pinned clock. A probe journey confirmed the page, runner and server clocks agreed at 2026-09-22T15:00:05Z and the page clock kept running. Gates (2026-09-25): `npm run check`, `npm run typecheck`, `npm run test:unit` (198: 51 API, 79 adapters, 11 contracts, 57 core), `npm run build` and `npm run test:browser` (31 Chromium journeys) passed. In the first full browser run in a fresh worktree, one mandate journey could not find its first page button within 5 s. That journey passed 3 of 3 times when repeated alone, and the next two full runs passed 31 of 31.
