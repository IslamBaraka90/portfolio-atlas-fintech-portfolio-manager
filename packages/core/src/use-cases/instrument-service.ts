...
export class InstrumentService {
  ...
  resolve(candidateId: string, context: CommandContext): Promise<InstrumentResolution> {
    return this.commands.execute("instrument.resolve", { candidateId }, context, async () => {
      ...
      if (reply.status === "unavailable")
        return { status: "unavailable", instrument: null, reasons: [], failure: reply.failure };
      ...
      if (reasons.length) return { status: "unresolved", instrument: null, reasons, failure: null };
      ...
      return { status: "resolved", instrument: record, reasons: record.warnings, failure: null };
    });
  }
  eligibility(input: {
    instrumentId: string;
